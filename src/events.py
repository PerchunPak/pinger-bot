...
class Events(Cog):
    ...
    async def on_ready(self):
        ...
        await pinger_bot_class.set_status(
            Status.online, "пинг превыше всего", ActivityType.playing
        )
    ...
    async def on_command_error(self, ctx, exception):
        ...
        if isinstance(exception, NotOwner):
            await ctx.send(
                f"Только мой Владелец, {self.bot.app_info.owner}, может использовать эту команду"
            )

        elif isinstance(exception, MissingRequiredArgument):
            await ctx.send(
                f"Не хватает необходимого аргумента `{exception.param.name}`"
            )
            ...
            if len(traceback) <= 1500:
                ...
                traceback = [
                    traceback[i : i + 1500] for i in range(0, len(traceback), 1500)
                ]
