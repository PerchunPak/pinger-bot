...
class Alias(Cog):
    ...
    async def alias(self, ctx, alias: str, ip: str):
        ...
        status, info = await self.metods_for_commands.ping_server(
            ip
        ...
        if len(database_server) != 0:
            ...
            if ctx.author.id != database_server["owner"]:
                ...
                embed.set_footer(
                    text=f'Для большей информации о сервере напишите "стата {name}"'
                )
            ...
            try:
                ...
                embed.set_footer(
                    text=f'Для большей информации о сервере напишите "пинг {name}"'
                )
