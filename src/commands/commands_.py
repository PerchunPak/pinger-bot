...
class MetodsForCommands:
    ...
    async def parse_ip(self, input_ip: str) -> ServerInfo:
        ...
        if len(ip_from_alias) != 0:
            ...
            alias = None if alias_from_ip == "" else alias_from_ip
    ...
    async def fail_message(ctx, ip: str, online: bool):
        ...
        """
        embed = Embed(
            title=f"Результаты пинга {ip}", description=f"\n\n**{'Онлайн' if online else 'Офлайн'}**", color=Color.red()
        )

        embed.add_field(
            name="Не удалось выполнить команду", value="Возможно вы указали неверный айпи, или сервер сейчас выключен"
        )
