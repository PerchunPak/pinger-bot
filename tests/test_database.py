...
class TestAddFunctions:
    ...
    async def test_add_server(self, database):
        ...
        server = await database.pool.fetch(
            "SELECT * FROM sunservers WHERE numip='127.0.0.10' AND port=25565"
        )
        assert len(server) > 0
    ...
    async def test_add_server_owner_id(self, database):
        ...
        server = await database.pool.fetch(
            "SELECT * FROM sunservers WHERE numip='127.0.0.11' AND port=25565"
        )
    ...
    async def test_add_ping(self, database):
        ...
        ping = await database.pool.fetch(
            "SELECT * FROM sunpings WHERE ip='127.0.0.12' AND port=25565"
        )
    ...
    async def test_add_alias(self, database):
        ...
        server = await database.pool.fetch(
            "SELECT * FROM sunservers WHERE numip='127.0.0.13' AND port=25565"
        )
    ...
    async def test_add_record(self, database):
        ...
        server = await database.pool.fetch(
            "SELECT * FROM sunservers WHERE numip='127.0.0.14' AND port=25565"
        )
...
class TestGetFunctions:
    ...
    async def test_get_server(self, database):
        ...
        right_answer = await database.pool.fetch(
            "SELECT * FROM sunservers WHERE numip='127.0.0.15' AND port=25565"
        )
    ...
    async def test_get_ip_alias(self, database):
        ...
        right_answer = await database.pool.fetch(
            "SELECT numip, port FROM sunservers WHERE alias='тест123';"
        )
        assert answer == right_answer
    ...
    async def test_get_pings(self, database):
        ...
        right_answer = await database.pool.fetch(
            "SELECT * FROM sunpings WHERE ip='127.0.0.23' AND port=25565;"
        )
        assert answer == right_answer
...
class TestAnotherFunctions:
    ...
    async def test_remove_too_old_pings(self, database):
        ...
        pings_right = [
            (IPv4Network("127.0.0.25/32"), 25565, time_23h, 3),
